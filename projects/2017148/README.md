...
### [Asciinema-Spark Shell&Pipelining](https://asciinema.org/a/368965)
...
Μέσο shell scripting και pipelining πιάνω ένα αρχείο csv, δίνω όνομα της σειράς που θα ήθελα να κάνω σε γράφημα και εμφανίζετε στο τέλος μέσο της εντολής spark.
...
echo "$2"
```
Η ιδέα είναι μέσο του ονόματος της σειράς που θέλω από ένα csv να εμφανίζονται τα αποτελέσματα.
...
```
Στο Script2 το πρώτο argument που θα δίνετε θα είναι το όνομα της σειράς που θέλουμε να φτιάξουμε το γραφημά μας άπο τα δεδομένα της και το δεύτερο θα είναι το όνομα του αρχείου. Έτσι ξέρω ότι $1 = όνομα σειράς και $2 = αρχείο. Το Script1 δεν είναι απαραίτητο αν έχεις ήδη κατεβασμένο το αρχείο csv.
Αρχικά παίρνω τους τίτλους της κάθε σειράς. Αντικαθιστώ οποιοδήποτε κενό με το χαρακτήρα "_" εφόσον μου δημιουργούσε προβλήματα. Τέλος το χωρίζω ανά κόμμα μέσα σε ένα πινάκα.
```
colnames=$(head -n 1 < $2)
colnames2=$( echo -e "$colnames" | tr  ' ' '_'  )
arr=(${colnames2//,/ })
...
Ακολούθως μέσο ενός for loop βλέπω το κάθε τίτλο ξεχωριστά σαν ένα item. Φεύγω το χαρακτήρα " για ευκολία στη σύγκριση στο if και δημιουργώ ένα counter για να ξέρω σε ποια σειρά είμαι.
```
etoimo=$(echo "$item" | tr -d '"')
	count=$((count+1))
```

Στο if ελένχω αν έχω τη σειρά που δόθηκε. Αν το argument 1, δηλαδή το όνομα της σειράς που δίνετε, είναι ίδιο με οποιοδήποτε από τους τίτλους μέσα στο πίνακα τότε το τυπώνουμε το αριθμό της για να το χρησιμοποιήσουμε στο επόμενο πρόγραμμα του pipe.
```
if [ "${etoimo}" == "${1}" ]; then
     echo "$count"
   fi
